...
<template>
  <div>
<div class="block">校园招聘</div>
  <template>
  <v-layout row>
    <v-flex xs12 sm6  md4 v-for="(item,index) in school" :key="index" style="margin-left:20px">
...
export default {
  ...
  methods:{
    getList(){
      this.$axios.get('/recruitments/school')
      .then(res=>{
        this.school=res.data.result
      })
    }
  }
}
