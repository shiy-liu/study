...
<template>
  <div>
<div class="block">社会招聘</div>
  <template>
  <v-layout row>
    <v-flex xs12 sm6  md4 v-for="(item,index) in social" :key="index" style="margin-left:20px">
...
export default {
  ...
  methods:{
    getList(){
      this.$axios.get('/recruitments/social')
      .then(res=>{
        this.social=res.data.result
      })
    }
  }
}
